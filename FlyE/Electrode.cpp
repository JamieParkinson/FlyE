...
void Electrode::import(std::unique_ptr<AcceleratorConfig> config) {
  for (int x = 0; x < config->x(); ++x) {  // Layers (excluding nx-th and first)
     ...
     basePath << config->datDirectory() << config->PAname()
          << "_E" << config->nElectrodes() - electrodeNumber_ + 1 << "_L"
          << x + 2 << "_";

     for (int d = 0; d < Physics::N_DIMENSIONS; ++d) {
       path << basePath << Physics::axes[d] << ".dat";
       blitz::Array<float, 3> thisDimension = this->extractComponent(float(), d, 3); // Get this dimension
